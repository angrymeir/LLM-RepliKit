...
# Limit processing to first 10 items in gpt35.py # TODO: remove after testing when doin experiment runs
...
# TODO store geneerated regexes in output so we can store them as evidence later
...
python3 Pass_at_k_Evaluation.py > /workspace/output/pass_at_k.txt # TODO: each fileline corresponds to an entry here ["Phi_Raw_Output","Phi_Refined_Output","Text_DaVinci_Raw_Output","Text_DaVinci_Refined_Output","GPT3.5_Raw_Output","GPT3.5_Refined_Output", "T5_Raw_Output","T5_Refined_Output"]

echo "[Step 4] Running DFA-EQ@K evaluation..."
python3 DFA_Equ_Evaluation.py > /workspace/output/dfa_eq_k.txt # TODO: at the beginning lots of "start x, end x" and result at the bottom of the file-> filter out in postprocess later
